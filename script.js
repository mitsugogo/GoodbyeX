document.addEventListener('DOMContentLoaded', () => {
    ...
    new MutationObserver(() => {
        ...
        if (titleElement) {
            ...
            else titleElement.textContent = titleElement.textContent.replace(' / X', ' / Twitter').replace('Xユーザーの', '');
        }
    ...
    new MutationObserver(() => {
        ...
        if (tweetButton && tweetButton.textContent != 'ツイート') {
        ...
        if (tweetSidebarButton && tweetSidebarButton.textContent != 'ツイート') {
            tweetSidebarButton.innerHTML = 'ツイート';
        }

        // Retweet Button
        const retweetButton = document.querySelector('div.css-175oi2r.r-16y2uox.r-1wbh5a2 > div > span');
        if (retweetButton && retweetButton.textContent == 'リポスト') {
            retweetButton.innerHTML = 'リツイート';
        }
        ...
        if (retweetedTweets && retweetedTweets.length > 0) {
            for (const retweetedTweet of retweetedTweets) {
                if (retweetedTweet && retweetedTweet.textContent.includes('リポスト')) {
                    retweetedTweet.innerHTML = retweetedTweet.innerHTML.replace('リポスト', 'リツイート');
                }
            }
        }
        ...
        if (notificationsTab) {
            ...
            if (articles && articles.length > 0) {
                for (const article of articles) {
                    ...
                    if (text && (text.textContent.includes(' ポスト') || text.textContent.includes('リポスト'))) {
                        text.innerHTML = text.innerHTML.replace(' ポスト', ' ツイート').replace('リポスト', 'リツイート');
                    }
                }
            }
        }
